/*
1、SetUI.css中的#MyDiv ul li a 中的#MyDiv与$("#MyDiv").initMath()中的#MyDiv要同名
*/

(function ($) {
    /*---------------------------------------$.extend-------------------------------------------*/
    $.extend({
        initSetGlobal: SetGlobal, //初始化全局对象
        iniEmbeded: SetEmbeded
    });

    //设置全局对象，以后可直接使用editorCode,小心变量名冲突
    var editorCode;
    var userParagraphEquation = true; //是不是用段落表达式
    var useDefault = true; //当创建表达式时，是不是添加缺省值
    var cutCopySaved = ""; //保存Cut、Copy的数据
    var useASCII = false; //是否用ASCII模式 
    var selectedDisplayEquation;  //下拉框的名称(Display Equation)
    var ForDivURL; //显示URL的DIV
    var ForDivMathML; //显示MathML的DIV
    ...
    function SetGlobal(aInputMath, aOutputMath, aEmbedCode, aDivURL,aDivMathML) {
        ...
        function DisplayMath() {
            MathJax.Hub.Queue(function () {
                var s = "\<script type=\"text/javascript\" src=\"https://www.hostmath.com/Math/MathJax.js?config=OK\"\>\</script\>";
                ...
                if (useASCII==false)
                {
                   aa= "https://www.HostMath.com/Show.aspx?Code=" + encodeURIComponent(m);
                   bb=math0;
                }
                else
                {
                   aa= "https://www.HostMath.com/Show.aspx?IsAsc=True&Code=" + encodeURIComponent(m);
                   bb=math1;
                }

                $("#" + ForDivURL).text(aa);
                $("#" + aEmbedCode).text(s);
                toMathML(bb, function (mml) { 
                         $("#" + ForDivMathML).text(mml);
                      });
    ...
    function SetEmbeded(aShowHideEmbeded, aDivEmbed, aSelectEquation, aShowHideURL,aShowHideMathML) {
        ...
        $("#" +aShowHideMathML).click(); //初始化时是隐藏的
        ...
        $("#" + aShowHideEmbeded).click(); //初始化时是隐藏的
        ...
        $("#" + aSelectEquation).change(function () {
            ...
            //下面是为了Fire editorCode的onChange()事件
            var m = editorCode.getValue();
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if ((typeof (e)).toLowerCase() === "string") {
                        if (e === "|") {
                            ...
                            var f = (function (e) {
                                return function () {
                                    todo[e](this, obj); //将按钮this传过去，也将obj传过去
                                    editorCode.focus();
                                };
                            })(e);
    ...
    var toolOptions = {
        ...
        toolbarText: {
            UseDefaultValue: "Enabled key in default value when click LaTeX icon",
    ...
    var toolAction = {
        ...
        UseDefaultValue: function (btn, obj) {
            useDefault = !useDefault; //这是一个全局变量

            var s = "#" + obj[0].id + " ul li:has(a.UseDefaultValue)";
            if (useDefault == true) {
                $(s).addClass("CheckedStatus");
                $(btn).attr("title", "Enabled key in default value when click LaTeX icon");
            } else {
                $(s).removeClass("CheckedStatus");
                $(btn).attr("title", "Disabled key in default value when click LaTeX icon");
            };
        },
        ...
        Asc: function (btn, obj) {
            ...
            if (useASCII == true) {
                ...
                $("#ForLatext").hide(); //这个名称和UI是紧耦合的
                $("#ForASC").show();    //这个名称和UI是紧耦合的
                $("#" + selectedDisplayEquation).attr("disabled", "disabled");
            } else {
                $(s).removeClass("CheckedStatus");
                $(btn).attr("title", "ASCIIMath mode disabled");
                $("#ForLatext").show(); //这个名称和UI是紧耦合的
                $("#ForASC").hide();   //这个名称和UI是紧耦合的
    ...
    /*--------------------------------------My Maths-------------------------------------------*/   
    /*--
    暂时不用的"amalg", "odot", "bigcirc", "dagger", "ddagger","ast"
    --*/  
    var mathOptions = {
        toolbar: [
    ...
    /*--------------------------------------My Logic-----------------------------------------*/
    /*
    暂时不用的"implies", "Leftrightarrow", "rightleftharpoons", "lhd", "rhd"    
    */
    var logicOptions = {
    toolbar: [
      ["neq", "leq", "geq",
    ...
    */
    ...
    /*--
    暂时不用的，"circledR","circledS","yen","emptyset","Re","aleph","complement","ldots",
    "cdots","cdot","nabla","vartriangle","Bbbk","vdots","ddots","lozenge","square",
    "smallsetminus","intercal","circleddash","neg",
    --*/    
    var symbolOptions = {
        toolbar: [
          [           
