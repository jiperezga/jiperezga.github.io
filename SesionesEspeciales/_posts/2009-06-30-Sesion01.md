...
si se tiene que `$\sigma_1^2=\sigma_2^2$`, o
...
si se tiene que `$\sigma_1^2\neq\sigma_2^2$`. En estos dos casos,
`$t_c$` se distribuirá como una `$t$` con `$(n_1+n_2-1)$` grados de
libertad o `$(n-1)$` grados de libertad, respectivamente.

#### Distribución muestral `$\chi^2$`

**Se usa para hacer inferencia sobre la varianza de una población**. Sea
`$X_1, X_2, \ldots, X_n$` una muestra aleatoria *iid* de una
