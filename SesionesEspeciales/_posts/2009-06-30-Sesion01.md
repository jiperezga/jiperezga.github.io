...
distribución Exponencial. Entonces, para probar esta distribución, será
...
prueba de bondad de ajuste, como por ejeplo la Kuiper, para saber si se
rechaza o no la hipótesis de interés.

``` r
library(MASS)
## Se hace el ajuste para encontrar el valor de los parámetros
parametros <- fitdistr(na.omit(datos$area_lote), densfun = "exponential")

## Se hace la prueba de bondad de ajuste
truncgof::v.test(na.omit(datos$area_lote), distn = "pexp", fit = list(rate = parametros$estimate))
```
