...
**Se usa para hacer inferencia sobre la varianza de una población**. Sea `$X_1, X_2, \ldots, X_n$` una muestra aleatoria *iid* de una distribución `$N(\mu,\sigma^2)$` de tamaño `$n$`, entonces se tendrá qué
`\begin{align*}
...
URL <- "https://raw.githubusercontent.com/jiperezga/jiperezga.github.io/master/Dataset/Codigos R/Intervalos.R"  # URL base de datos
...
```{r}
IntDosMedias(datos1 = datos$preciovtax[datos$estrato == 2], datos2 = datos$preciovtax[datos$estrato == 4], nivel.conf = 0.90)
```
