...
La media, varianza y autocovarianza del proceso AR(p), bajo la condición de estacionaridad, se obtienen mediante las euaciones
`\begin{align*}
\mathbb{E}(\widetilde{Y}_t) & = 0 \\
...
```{r echo=FALSE}
library(kableExtra)
tabla1 <- rbind(c("ARMA(p,q)", "Comportamiento irregular en los primeros rezagos (1, ... , q) con q picos. Convergencia a cero para k>q.", "Decrece (aproximadamente con exponenciales atenuados y ondas sinusoidales). No cae rápidamente a cero"))
colnames(tabla1) = c("Proceso", "Función de autocorrelación (ACF)", "Función de autocorrelación parcial (PACF)")
