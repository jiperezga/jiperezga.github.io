...
con `$\varepsilon_t \sim RB(0, \sigma_{\varepsilon}^2)$`, entonces, si una de las `$p$` raicez de la ecuación `$\Phi_p(z) = 1 - \sum_{j=1}^p=\phi_j z^j = 0$` es `$z = 1$` entonces se tendrá que `$Y_t$` tiene una raíz unitaria.
...
con `$\gamma = \phi_1 - 1$`. Entonces is hay raices unitarias se cumplirá que `$\phi_1 = 0$` o equivalentemente `$\gamma=0$`.
...
La prueba Dickey-Fuller puede realizarse en <tt>R</tt>, mediante la función `adf.test()` de la librería `tseries` o de la librería `aTSA`.
