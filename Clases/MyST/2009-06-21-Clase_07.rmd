...
Para poder decidir cuál es el modelo más apropiado, es necesario que la serie de tiempo sea estacionaria. De no ser estacionaria, podría requerirse el empleo de **diferenciaciones** para convertir la serie original a una serie estacionaria. A partir del proceso de diferenciación se identifica el orden *d*, del modelo a estimar.
...
Luego de la etapa de identificación, se obtendrá información sobre los ordenes de *p*, *d* y *q*, los cual permitirá estimar los coeficientes de los términos auto-regresivo y de medias móviles incluidos en el modelo. Para la estimación del modelo se emplea generalmente mínimos cuadrados ordinarios lineales, o no lineales.
...
Si se evidencia que los residuales no son un proceso puramente aleatorio, se requiere que se modifique el modelo, y por ello se volverá a la etapa de identificación para reformular el modelo hasta que los errores sigan un proceso puramente aleatorio.

### Pronóstico

Finalmente, una vez se obtiene el modelo más adecuado, es posible realizar pronósticos para la serie, en donde, es importante anotar, que si la serie original fue diferenciada, ésta debe ser retornada a sus valores originales, una vez los valores pronosticados son calculados. 
...
Entonces, una serie de tiempo, está dada por una sucesión de observaciones generadas al tomar un único valor de cada una de las variables aleatorias `$Y_t$`, que conforman el proceso estocástico.

Además, para la realización del modelamiento estadístico de una serie de tiempo, es necesario identificar la estructura probabilística del proceso estocástico al que corresponde la serie, y para ésto, es necesario conocer la distribución conjunta de las variables que conforman la serie de tiempo, lo cual es difícilmente posible, ya que para ello, se requeriría de múltiples realizaciones del proceso estocástico.

Debido a ello, y con el fin de dar una salida al problema, **se supone** que cada variable `$Y_t \sim N(\mu_t,\sigma_t^2)$` y `$\mathbf{Y}\sim N_T(\boldsymbol{\mu}, \boldsymbol{\Sigma})$`, siendo `$\mathbf{Y}=(Y_1,Y_2,\ldots,Y_T)'$` un vector que representa la variable aleatoria conjunta, `$\boldsymbol{\mu}=(\mu_1,\mu_2,\ldots,\mu_T)$` un vector de esperanzas de las variables aleatorias, y 
...
la matriz de covarianzas de las variables aleatorias. 
...
\end{align*}`
4. Otra propiedad que pueden cumplir los procesos estacionarios en covarianza, son las **propiedades ergódicas**, las cuales se refieren a que entre mayor sea la longitud de la serie de tiempo, mejores serán las estimaciones de los parámetros estimados, es decir, se se tendrán estimadores consistentes de los parámetros.
<ol type="a">
...
donde, `$\gamma(0)$` representa la varianza de la variable `$Y_t$`, `$\gamma(k)$` la covarianza de entre las variables `$Y_t$` y `$Y_{t+k}$`, y `$k$` es el número de rezagos que se emplearán para el cálculo de las autocorrelaciones.
...
La importancia de la ACF en la metodología Box-Jenkins, radica en que permite determinar si la serie de tiempo de interés, es o no un proceso estacionario en covarianza, y para ello se emplea la siguiente prueba de hipótesis 
`\begin{align*}
 H_0: \rho(k)=0 \\
 H_1: \rho(k)\neq 0
\end{align*}`
...
en donde, valores `$|\hat{\rho}(k)|> Z_{\frac{\alpha}{2}}/\sqrt{T}$` llevarán al rechazo de la hipótesis nula a favor de la alternativa. También recuerde que, si ninguna de las autocorrelaciones es significativamente diferente de cero, la serie es esencialmente **ruido blanco**.
