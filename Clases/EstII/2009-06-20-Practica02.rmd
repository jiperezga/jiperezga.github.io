...
`\begin{align*}
	\hat{\sigma} = \sum_{i=1}^{n}\frac{X^2_i}{n}
\end{align*}`
<ol type ="a">
  <li> Muestre si dicho estimador es un estimador insesgado de $\sigma^2$, de no serlo, calcule su sesgo y pruebe si es un estimador asintóticamente insesgado.
  </li>
...
18. Sea `$X_1, X_2, \dots, X_k$` una muestra aleatoria `$iid$` de una variable aleatoria discreta Binomial tal que `$X\sim Bin(n,p)$` y suponga los estimadores
`\begin{align*}
  \hat{p}_1&=\frac{X}{n}; \\    
...
<ol type ="a">
  ...
  <li> Cuál de los dos estimadores es más eficiente, si solo se tiene en cuenta la varianza.
  </li>
  <li> Cuál de los dos estimadores es más eficiente, si se tiene en cuenta el sesgo del estimador.
  </li>
...
`\begin{align*}
 	f(x)= \frac{\alpha x^{\alpha-1}}{3^\alpha}; \quad 0\leq x\leq\theta; \quad \alpha >0; \quad \theta>0
\end{align*}`
Si `$\theta=3$` demuestre que `$\mathbb{E}(X)=3\alpha/(\alpha+1)$` y encuentre el estimador de `$\alpha$` por el método de los momentos.
