...
2. Sea `$X_1, X_2, \ldots, X_n$` una muestra aleatoria `$iid$` de una distribución Binomial con parámetros desconocido `$p$` y parámetro `$m$` conocido o desconocido, con función de distribución dada por
`\begin{align*}
f(x) = \left(\begin{array}{c}m\\ x\end{array}\right)p^x(1-p)^{m-x} \text{ para } x=0,1,\ldots, m; \quad 0\leq p \leq 1
\end{align*}`
<ol type ="a">
  <li> Calcule el estimador para el parámetro $p$ por el método de los momentos, si se supone que $m$ es conocida.
  </li>
  <li> Calcule el estimador para los parámetros $m$ y $p$ por el método de los momentos.
  </li>
  <li> Calcule el estimador de máxima verosimilitud para el parámetro $p$, si se supone $m$ conocida y pruebe si el estimador encontrado efectivamente maximiza la función. 
  </li>
...
18. Sea `$X_1, X_2, \dots, X_n$` una muestra aleatoria `$iid$` de una variable aleatoria discreta Binomial tal que `$X\sim Bin(m,p)$` y suponga los estimadores
`\begin{align*}
  \hat{p}_1&=\frac{X}{n}; \\    
