...
opts_chunk$set(echo=TRUE,
               ...
               message=FALSE,
               warning=FALSE,
               fig.path = paste0("../../EstadisticaII/images/", "Clase14"),
...
En donde observamos que requerimos la media muestral $\bar{X}=327$, la desviación estándar poblacional $\sigma=75.52134$, el tamaño de muestra $n=5000$, el nivel de significancia $\alpha=0.05$ y el valor crítico $-Z_{\alpha=0.05}=-1.644854$. Al reemplazar en el estadístico de prueba tenemos que
$$\begin{align*}
  Z_c &= \frac{327-330}{75.52134/\sqrt{5000}} \\
      &= -2.808902
\end{align*}$$

En esta ocasión se decide calcular el P-valor para tomar la decisión, se tendrá que
$$\begin{align*}
...
en donde al ser el valor crítico $Z_{\frac{\alpha}{2}=0.05}=1.644854$, se tendrá que la región crítica está dada por
$$\begin{align*}
...
En donde, para realizar el cálculo requerimos de las media muestrales fueron $\bar{X}_C=43.5$ y $\bar{X}_S=40.8$, las varianzas poblacionales $\sigma^2_C=28$ y $\sigma^2_S=20$, los tamaños de muestrales $n_C=12$ y $n_S=15$. Al reemplazar en la ecuación del estadístico de prueba tendremos que
$$\begin{align*}
...
Entonces como el P-valor es mayor al nivel de significancia del $5\%$ NO se rechazará la hipótesis nula, y por tanto se tendrá evidencia suficiente para concluir que el promedio de ventas de las personas que recibieron capacitación NO es superior al promedio de ventas de las personas que no recibieron capacitación.
</p>
</main>
...
Ahora, si empleamos la región crítica para observar si se rechaza o no la hipótesis, tendremos que 
$$\begin{align*}
  RC&:\{Z|Z>Z_\alpha\}\\
  RC&:\{Z|Z>1.03643\}
\end{align*}$$
...
Ahora, si realizamos el cálculo de la región crítica para observar si se rechaza o no la hipótesis establecida, tendremos que 
$$\begin{align*}
...
En donde, se decide emplear el P-valor para observar si hay o no evidencia a favor de la hipótesis nula obteniendo que 
$$\begin{align*}
  P-valor &= \mathbb{P}(Z>Z_c) \\
          &= \mathbb{P}(Z>-1.369306)\\
