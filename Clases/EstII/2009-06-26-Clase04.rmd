...
Sea `$X_2, X_3, X_4, \ldots$` una secuencia de variables aleatorias, y sea `$X$` una variable aleatoria definida en el espacio muestral, entonces, si `$F_{X_{n}}$` y `$F_{X}$` son las funciones de distribución acumulada de `$X_n$` y `$X$` respectivamente, se tendrá que `$X_n$` converge en distribución a `$X$`, si
`\begin{align*}
...
<main id="botoncito2"> 
<h3 data-toc-skip> Ejercicio </h3> 
<p>Sea $X_1, X_2, \ldots$ una secuencia de variables aleatorias distribuidas, tal que 
$$\begin{align*}
