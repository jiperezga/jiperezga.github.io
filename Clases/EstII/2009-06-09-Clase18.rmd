...
`\begin{align*}
 \beta_0 &\sim N\left(\beta_0, \left[\frac{1}{n} + \frac{\bar{X}^2}{S_{xx}}\right]\sigma^2\right)  \\
 \beta_1 &\sim N\left(\beta_1, \frac{\sigma^2}{S_{xx}}\right)
\end{align*}`
...
`\begin{align*}
 (y_i - \bar{Y}) = (\hat{y} - \bar{Y}) + (\underbrace{y_i - \hat{y_i}}_{e_i})
\end{align*}`
