...
```{r echo=FALSE, eval=F}
library(kableExtra)
tabla <- data.frame(rbind(c("", "", "$\\mathbf{\\text{Reemplazo}}$", ""), 
                          c("", "", "$\\mathbf{\\text{Sí}}$", "$\\mathbf{\\text{No}}$"), 
                          c("$\\mathbf{\\text{Orden}}$", "$\\mathbf{\\text{Sí}}$", "$n^r$", "$_nP_r= n(n-1)(n-2)\\ldots(n-r+1)=\\frac{n!}{(n-r)!}$"), 
                          c("", "$\\mathbf{\\text{No}}$", "$_{n+r-1}C_r = \\left(\\begin{array}{c}n+r-1\\\\ r\\end{array}\\right) = \\frac{(n+r-1)!}{r!\\;(n-1)!}$", "$_nC_r = \\left(\\begin{array}{c}n\\\\ r\\end{array}\\right) = \\frac{n!}{r!\\;(n-r)!}$")))
