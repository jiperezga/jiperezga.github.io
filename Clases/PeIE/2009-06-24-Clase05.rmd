...
Existen diferentes funciones que permiten realizar resúmenes numéricos en <tt>R</tt>, entre ellas se destaca la función `numSummary()` de la librería `RcmdrMisc` y la función `describe()` de la librería `psych`, que permiten realizar resúmenes de variables tipo numéricas. 
...
numSummary(datos$`Cost of Production (/ Quintal) C2`, statistics = c("mean", "sd", "se", "IQR", "quantiles", "cv", "skewness", "kurtosis"), type = "3", quantiles = c(0, .25, .5, .75, 1)) 

library(psych) # Carga librería pysch
# Forma básica pysch
describe(datos$`Yield (Quintal/ Hectare)`)
# Forma avanzada pysch
describe(datos$`Cost of Production (/ Quintal) C2`, na.rm = TRUE, interp = FALSE, skew = TRUE, ranges = TRUE, trim = 0.2,
