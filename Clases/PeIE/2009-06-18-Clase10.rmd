...
`\begin{align*}
\mathbb{E}(X)=\mu \quad \quad Var(X)=\sigma
\end{align*}`
...
`\begin{align*} 
f(z) = \begin{cases} \frac{1}{\sqrt{2\pi}}e^{-\frac{x^2}{2}} & -\infty<x<\infty \\ 0 & \text{en otro caso} \end{cases}
\end{align*}`

### Propiedades
Si `$X\sim N(\mu, \sigma^2$`, se tendrá que `$Z=\frac{x-\mu}{\sigma^2}\sim N(0,1)$`, y por tanto, si `$a$` y `$b$` son constantes, entonces 
