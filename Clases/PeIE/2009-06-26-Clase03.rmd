...
Sea `$X$` una variable aleatoria con *fmp* `$p(x)$`, entonces si `$g(X) = (X - \mathbb{E}(X))$`, se tendrá que la varianza de `$X$` que se denota `$Var(X)$` o `$\sigma^2$` estará dada por
`\begin{align*}
Var(X) = \mathbb{E}\left[\left(X-\mathbb{E}(X)\right)^2\right]=\sum_x(x-\mathbb{E}(X))^2p(x)
\end{align*}`

Una alternativa para el cálculo de la `$Var(X)$`, la cual puede ser demostrada a partir de la ecuación anterior es
