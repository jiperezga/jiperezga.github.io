...
función `bp.test()` de la librería `lmtest`, o su versión mejorada
conocida como la prueba Cook-Weisberg, que puede realizarse mediante la
función `ncvTest()` de la librería `lmtest`.
...
realizarse en <tt>R</tt>, mediante la función `gq.test()` de la librería
`lmtest`.
