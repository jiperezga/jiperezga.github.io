...
En situaciones en las cuales se observa que la serie temporal presenta
un componente estacional, **se tendrá que la serie de tiempo no es
estacionaria**, lo cual hace que los métodos anteriormente presentados
para modelar las series no sea apropiada.
...
`\begin{align*} \Phi_{p_s} (L^s) \Phi_p (L) \Delta^D_s \Delta^d \widetilde{Y}_t = \Theta_{q_s} (L^s) \Theta_q (L)\varepsilon_t \end{align*}`
...
`\begin{align*} \Phi_{p_s}(z^s) = 1 - \sum_{j=1}^{p_s}\phi_{s,j} z^{js} \\ \Theta_{q_s}(z^s) = 1 + \sum_{j=1}^{q_s}\theta_{s,j} z^{js} \end{align*}`
...
1**, y `$\Delta^d=(1-L^s)^D$` es el operador de diferencias
estacionales.
...
`\begin{align*} \Phi_{p_s} (L^s) \Delta^D_s \widetilde{Y}_t = \Theta_{q_s}(L^s) \varepsilon_t \end{align*}`

si `$D=0, Q=0$` se tendrá un modelo AR(P)\[s\], donde
`\begin{align*} \Phi_{p_s} (L^s) \widetilde{Y}_t & = \varepsilon_t \\                                  & = \phi_0 + \phi_{1,s}Y_{t-s} + \phi_{2,s}Y_{t-2s} + \ldots + \phi_{P,s}Y_{t-Ps} + \varepsilon_t \end{align*}`
...
`\begin{align*} \widetilde{Y}_t & = \Theta_{q_s}(L^s) \varepsilon_t \\                                  & = \varepsilon_t - \theta_{1,s} \varepsilon_{t-s} - \theta_{2,s} \varepsilon_{t-2s} - \ldots - \theta_{Q,s} \varepsilon_{t-Qs} \end{align*}`
...
por
`\begin{align*} \Phi_{p_s} (L^s) \Phi_p (L) \widetilde{Y}_t = \Theta_{q_s}(L^s) \Theta_q (L)\varepsilon_t \end{align*}`
con `$\Phi_{p_s} (z^s)$` y `$\Phi_p (z)$` los polinomios de rezago
autorregresivo regular y estacional, cada uno **sin raíces unitarias en
`$z$` y `$z^s$`**, respectivamente.

### Raices unitarias estacionales

Comos se señala en Guerrero ([2003](#ref-Guerrero2003), p. 216), existen
...
función `ch.test()` de la librería `uroot`. Se recomienda que para
aplicar el test Canova-Hansen se elimine la tendencia de la serie en
caso de existir, lo cual puede hacerse eliminando la componente de
tendencia estimada `$T_t$` mediante la función `stl()` vista en la
[Clase
...
de raíces unitarias estacionales de pruebaOsborn,Chui, Smith, y
Birchenhall, la cual se basa en una prueba de regresión dada por
`\begin{align*} \Delta\Delta_sY_t = \beta_1\Delta_sY_{t-1} + \beta_2 Y_{n-s} + Z_t \end{align*}`
...
`\begin{align*} \Delta\Delta_sY_t = \beta_1 (I+L+\ldots+L^{s-1})\Delta Y_{t-1} + \beta_2\Delta Y_{n-s} + Z_t \end{align*}`

A partir de esta regresión se trata de probar el siguiente juego de
hipótesis
`\begin{align*} H_0: Y_t \sim I(1,1) \\  H_1: Y_t \sim I(0,0) \vee I(0,1) \vee I(1,0) \end{align*}`
...
estadísitico de prueba ae por debajo del valor crítico se rechaza la
