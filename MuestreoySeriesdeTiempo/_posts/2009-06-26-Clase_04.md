...
-   **Serie estacionaria en media:** Promedios simples, Promedios
    móviles - Suavizamiento exponencial simple
-   **Serie con tendencia:** Promedios móviles - Suavizamiento
    exponencial de Holt-Winters - Suavizamiento exponencial de Brown
...
observaciones, y que su valor se actualiza periodicamente a medida que
se van agregando nuevas observaciones al cojunto de datos, pero dado que
esta medida es el promedio de todo el conjunto de datos históricos de la
serie, ésta no tendrá cambios significativos cuando ingresen nuevas
observaciones.
...
se hace de la forma `\begin{align*} \hat{Y}_{t}=PS_t \end{align*}`
...
`\begin{align*} \hat{Y}_{t}= \begin{cases} PMU_t & \text{ en el caso de promedios móviles unilaterales} \\ PMB_t & \text{ en el caso de promedios móviles bilaterales} \end{cases} \end{align*}`
...
`\begin{align*} \hat{Y}_{t+1}=\alpha Y_{t} + (1-\alpha)\hat{Y}_{t} \end{align*}`
...
Es de anotar que en la suavización exponencial simples, el ajuste de la
primera observación se puede hacer de diferentes formas, entre las
cuales destaca
