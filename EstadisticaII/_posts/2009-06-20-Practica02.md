...
30. Sea `$X_1, X_2, \dots, X_n$` una muestra aleatoria `$iid$` de una
    ...
    `\begin{align*}     f(x)= \frac{\alpha x^{\alpha-1}}{3^\alpha}; \quad 0\leq x\leq\theta; \quad \alpha >0; \quad \theta>0 \end{align*}`
    Si `$\theta=3$` demuestre que `$\mathbb{E}(X)=3\alpha/(\alpha+1)$` y
    encuentre el estimador de `$\alpha$` por el método de los momentos.
