...
8.  Un granjero cultiva peras en su finca para posteriormente venderlas
    ...
    entrega de `$1000$` manzanas, el `$10\%$` están echadas a perder.
    Con el fin de aceptar dicho pedido, en la minorista realizan una
...
11. Dado que ya hemos lanzado al aire una moneda balanceada diez veces y
    no obtuvimos caras, ¿cuál es la probabilidad de que debemos lanzarla
    al menos dos veces más para obtener la primera cara?
...
22. La facultad de ciencias económicas de cierta universidad realiza a
    ...
    seleccionan al azar 200 de los encuestados y se les pide su opinión,
    ¿Cuál es la probabilidad de que más de 60 pero menos de 80 no estén
    conformes con su carrera?.
23. Suponga una variable aleatoria discreta `$X$` con función de masa de
    probabilidad dada
...
29. Suponga que se realiza un estudio estadístico y se encuentra que el
    ...
    <li>
    Si en los 10 primeros minutos no ha llegado ningún cliente, ¿Cuál es
    la probabilidad de que llegue un cliente en los próximos 10 minutos?
    </li>
    </ol>
...
31. En una noche de tragos dos estudiantes de Estadística I van a un bar
    ...
    dardos y que la probabilidad de golpear en la diana sea de `$30\%$`.
    Si `$X$` es la variable aleatoria del número de dardos que golpean
    la diana. ¿Cuál es la probabilidad de que golpeen al menos `$160$`
    pero a lo más `$170$` dardos?
...
33. La cantidad diaria de vasos de café que toma un profesor
    ...
    tintos que toma un profesor se encuentre a más de una desviación
    estándar estándar de la media? Compare el resultado con el obtenido
...
36. Dos personas, por turnos, tiran un dado imparcial hasta que una de
    ellas lanza un 6. La persona A tiró primero, la B en segundo, A en
    tercero y así sucesivamente. En vista de que la persona B tiró el
    primer `$6$`, ¿cuál es la probabilidad de que B obtenga el primer
    `$6$` en su segundo tiro (es decir, en el cuarto tiro total)?
37. En una industria automotríz, se sabe que diez por ciento de los
    ...
    el segundo intento?
38. Aproximadamente `$4\%$` de las obleas de silicio producidas por un
    fabricante tienen menos de dos defectos grandes. Si `$Y$` representa
    al número de defectos por oblea ¿qué proporción de las obleas tiene
