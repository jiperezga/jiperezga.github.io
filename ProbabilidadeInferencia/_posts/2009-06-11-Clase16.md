...
decir,
`\begin{align*} RC:\{\text{valores del estadístico de prueba que conducen a rechazar }H_0\} \begin{align*}´ Por ejemplo, si se emplea la afirmación del médico sobre el peso promedio de los estudiantes, se tendrá el siguiente juego de hipótesis`\\begin{align*}
H\_0: = 73 kg \\ H\_1: 73 kg
\\begin{align*}`Supongamos una región crítica arbitraria, tal que`\\begin{align*}
RC:{{X}|{X}&lt;71 {X}&gt;75} \\begin{align*}\`
...
`\begin{align*} \text{P-valor }<\alpha \Rightarrow \text{ Rechazar } H_0 \begin{align*}`
...
define como
`\begin{align*} \alpha = \mathbb{P}(\text{Rechazar }H_0|H_0 \text{ es verdadera}) \begin{align*}`
...
como
`\begin{align*} \beta = \mathbb{P}(\text{No rechazar }H_0|H_0 \text{ es falsa}) \begin{align*}`
Este valor es imposible de calcular a menos de que se tenga una
hipótesis alternativa específica, tal que
`\begin{align*} H_1:\theta = \theta_1 \begin{align*}` con `$\theta_1$`
un valor específico a probar.
