...
1.  Se encuentra que la concentración promedio de zinc que se obtiene en
    ...
    de `$2.6$` gramos por mililitro. Si la función de probabilidad es
    ...
    río se encuentre entre \(2.2\) y \(2.3\) gramos por mililitro.
    </li>
    <li>
...
5.  Para un experimento de pruebas psicológicas se seleccionan al azar
    ...
    <ol type="a">
    <li>
    Calcule la probabilidad de el tiempo de reacción promedio para todos
    los individuos sea a lo más de \(6\) segundos.
    </li>
    <li>
...
6.  Un investigador de la afirma que la esperanza de vida de los ratones
    se puede extender hasta en `$25\%$` cuando se reduce aproximadamente
    `$40\%$` de las calorías de su dieta desde el momento en que son
    destetados, en donde, la dieta restringida se enriquece hasta
    niveles normales con vitaminas y proteínas. Si se supone que a
    partir de estudios previos se sabe que la desviación estándar del
    aumento de vida de los ratones es de `$5.8$` meses, ¿cuántos ratones
    se deberían incluir en la muestra para tener un `$99\%$` de
    confianza en que la vida media esperada de la muestra estará dentro
    de 2 meses a partir de la media de la población para todos los
    ratones sujetos a la dieta reducida?
...
8.  Dos marcas de refrigeradores, denotadas por `$A$` y `$B$` poseen
    ...
    <ol type="a">
    <li>
    Calcule con un nivel de confianza del \(98\%\) un intervalo de
    confianza para la diferencia entre las proporciones de falla, dentro
    del periodo de garantía, para todos los refrigeradores de las marcas
    \(A\) y \(B\).
    </li>
    <li>
    ¿Podría concluirse que hay evidencia suficiente para afirma que
    existen diferencia significativas entre las proporciones de fallas,
    dentro del período de garantía, para las dos marcas de
    refrigeradores? ¿Por qué?
    </li>
    </ol>
9.  ¿Está menguando el romance de los estadounidenses con el cine?, En
    ...
    total de de adultos que piensan que el cine está mejorando.
    </li>
    <li>
...
10. Los administradores de un hospital deseaban estimar el número
    ...
    <ol type="a">
    <li>
    Calcule la probabilidad de que la varianza de la población de
    pacientes de la cual se extrajo la muestra, tenga un valor superior
    a \(8_{días^2}\) .
    </li>
    <li>
...
13. Es frecuente que encuestadores por teléfono entrevisten entre
    ...
    <ol type="a">
    <li>
    Si se han de entrevistar \(1000\) hombres y \(1000\) mujeres, ¿con
    cuánta precisión podría usted estimar la diferencia en las
    proporciones que piensan que el rendimiento de sus equipos de
    atletismo tiene un impacto positivo en la percepción del público
    acerca del prestigio de las instituciones? Encuentre un límite para
    el error de estimación.
    </li>
    <li>
    Supongamos que usted estuviera diseñando la encuesta y desea estimar
    la diferencia en un par de proporciones, correcta a no más de
    \(0.02\), con probabilidad \(0.9\). ¿Cuántas entrevistas deben
    incluirse en cada muestra si suponemos que éstas son iguales?
    </li>
    </ol>
14. Las calificaciones del Examen de Evaluación Escolar (SAT por sus
    ...
    calificaciones del SAT verbal para alumnos de último año de
    preparatoria urbana.
    </li>
    <li>
...
17. Dos nuevos medicamentos se dieron a pacientes con hipertensión. El
    ...
    en la presión sanguínea sean iguales, suponiendo que las mediciones
