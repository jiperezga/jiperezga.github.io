...
4.  Sea el espacio muestral `$S = \{x | 1 < x < 12\}$` definido como el
    ...
    horas, el evento `$Y = \{x | 1.5 < x < 8\}$` representa el tiempo
    que tardan los estudiantes que estudiaron entre `$2$` y `$5$` horas,
    y `$Z = \{x | 7 < x < 12\}$`, el tiempo que tardan los estudiantes
    que estudiaron menos de `$2$` horas. Dado lo anterior, encuentre e
